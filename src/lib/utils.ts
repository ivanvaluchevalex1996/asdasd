import type { TCountry, TCountryListItem } from '@/types/types';
...
// ——— flags ———
export const getFlagPath = (iso: string) => `/api/flag/${iso.toLowerCase()}`;

// ——— data shaping (для уменьшения размера props на главной) ———
export const toCountryListItem = (c: TCountry): TCountryListItem => ({
  id: c.id,
  country: c.country,
  iso: c.iso,
  url: c.url,
  price: { amount: c.price.amount, symbol: c.price.symbol },
});
